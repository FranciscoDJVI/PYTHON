...
class Credentials:
    ...
    def generate_user_db(self):
        ...
        character_1 = user_info["name"][0:3]
        character_2 = user_info["lastname"][0:3]

        user_db.user_name_db = character_1 + character_2

        # Solicitar contraseña al usuario
        user_db.user_password_db = input("Cree una contraseña de max. 8 caracteres: ")

        # Comprobamos si la contraseña tiene 8 caracteres.
        correct: bool = False

        while correct == False:
            try:
                if not len(user_db.user_password_db) == 8:
                    print("la contraseña debe contener max. 8 caracteres")

                if user_db.user_password_db == "":
                    print("la contraseña no puede ser un espacio vacio")

                if len(user_db.user_password_db) == 8:
                    correct = True
                    break

                user_db.user_password_db = input("Ingrese nuevamente la contraseña: ")
